...
class SkeletonShaderBundle:
    def __init__(self, lines):
        ...
        #try:
        gs= OpenGL.GL.shaders.compileShader(sh.geometry_sphere, GL_GEOMETRY_SHADER)
        vs= OpenGL.GL.shaders.compileShader(sh.vertex_sphere, GL_VERTEX_SHADER)
        fs=OpenGL.GL.shaders.compileShader(sh.fragment_sphere, GL_FRAGMENT_SHADER)
        print(glGetShaderInfoLog(vs))
        print(glGetShaderInfoLog(gs))
        print(glGetShaderInfoLog(fs))
        self.shader_sphere = OpenGL.GL.shaders.compileProgram(
            vs,
            gs,
            fs,
            #validate=False
    ...
    def _init_buffers(self, vao, vbo_points, vbo_colors, data_lines, data_color, size):
    ...
    def mark_points(self, points):
        ...
        self.colors = [self.context.data_context.settings.POINT_COLOR] * self.points_size
    ...
    def add_points_color(self,points,colors):
        self.marked_points.extend(points)
        self.points_size += len(points)
    ...
    def set_lines_color(self,lines,color):
        ...
        for (x,y) in lines:
            ...
            if not_found==2:
                ...
                self.line_colors.append((color,color))
        #         print('not found',(x,y))
        #         not_found_times+=1
...
class OpenGLCanvas(glcanvas.GLCanvas):
    def __init__(self, parent):
        ...
        self.line_width = self.context.data_context.settings.LINE_WIDTH
        self.point_size = self.context.data_context.settings.DOT_SIZE
        self.labels=[]
        ...
        self.pi_gradient = np.array(
            [(200, 200, 200, min(2, max(0, i // 10))) for i in range(255)]
        ).astype(np.uint8)
        ...
        self.scale=2.0
        self.InitGL()
        self.arrows_drawer=ArrowsDrawer()

        self.additional_draw=[]

        self.label_x_offset=-7
        self.label_y_offset=-12
        self.label_dir_mul=0.015

        if TEST:
            self.test_list = []
            self.init_concentration(
                "C:\\Users\\Andrzej\\Downloads\\animation data\\xp47_73\\dr73-1_pi.tif",
                "C:\\Users\\Andrzej\\Downloads\\animation data\\xp47_73\\dr73-1_pi.tif",
                downscale=False,
            )
    ...
    def _lines_coord_microns_to_stack(self,lines):
        return [tuple(map(self._micron_coord_to_stack,line)) for line in lines]
    ...
    def graph_to_lines_smoothed(self):
        lines,tangents = self.graph_to_lines()
        smoothed_lines = []
        points= set()
        def p_fun(t,a,b,m,i):
            p = (2*t**3 - 3*t**2+1) *a + (t**3 - 2*t**2+t)*m[i]+((-2)*t**3 + 3*t**2)*b+(t**3 - t**2)*m[i+1]
            return p
        for (a, b),(m1,m2) in zip(lines,tangents):
            distance = int(ceil(1.2*sum((x-y)**2 for x,y in zip(self._opengl_coord_to_stack(a),self._opengl_coord_to_stack(b)))**0.5))
            points.add(a)
            points.add(b)
            a = np.array(a)
            b = np.array(b)
            ...
            for i in range(1,distance):
                next_point = p_fun(i/distance,a,b,M,0)
                smoothed_lines.append((last_point,next_point))
                points.add(tuple(next_point))
                last_point=next_point
            smoothed_lines.append((last_point,b))

        return smoothed_lines,points
    ...
    def point_line_distance(self, point, line):
        ...
        for start, end in line:
            distance=min(distance,self.point_line_segment_distance(point,start,end))
        return distance * self.unit_to_microns_coef

    def point_line_segment_distance(self,point,start,end):
        a = np.array(point) - np.array(start)
        b = np.array(end) - np.array(start)
        c = np.array(point) - np.array(end)
        proj_len = np.dot(a, b) / np.linalg.norm(b)
    ...
    def line_line_distance_points(self,points):
        point=-1
        line = []
        for i in range(1,4):
            _line = find_primary(self.skeleton_graph,points[0],points[i])
            if len(_line)>0:
                line=_line
                point=i
                break
        other = [1,2,3]
        other.remove(point)
        return self.line_line_distance((points[0],points[i]),(points[other[0]],points[other[1]])),(points[0],points[i]),(points[other[0]],points[other[1]])
    ...
    def OnMouse(self, event):
        ...
        if event.ButtonDown(wx.MOUSE_BTN_LEFT):
            ...
            if self.modes.get("find_any_point"):
                ...
                _f = self.modes.get("find_any_point")
        ...
        if (event.GetWheelRotation()) != 0:
            ...
            self.context.graphic_context.set_zoom_scale_field(self.scale)
            self.scale+=max(0.001 * event.GetWheelRotation(),-self.scale/10)
            self.Refresh()
            # print('draging')
        if self.counter >= 1:
            self.counter = 0
        ...
        if not self.init:
            ...
            if not self.concentration_loaded and self.scaling_ratio==None:
                ...
                if self.context.data_context.settings.DRAW_ARROWS:
                    self.arrows_drawer.draw()
                for program,draw_func in self.additional_draw:
                    glUseProgram(program)
                    self.init_uniforms(program)
                    draw_func()
                self.SwapBuffers()
                self.dc = wx.PaintDC(self)
                self._draw_labels()
    ...
    def to_screen_space(self,pos : Vector4) :
        vp = self.get_vp_matrix()
        pos=pos@self.model_matrix@vp
        return Vector3([pos[0] / pos[3], pos[1] / pos[3], pos[2] / pos[3]])

    def _draw_labels(self):
        center = self.context.data_context.values["center"]
        center = self.to_screen_space(Vector4([*self._stack_coord_to_opengl(center), 1]))
        for label,point in self.context.data_context.get_labels():
            # uncoment to turn of 
            #continue
            if label:
                pos = Vector4([*self._stack_coord_to_opengl(point), 1])
                pos = self.to_screen_space(pos)
                center_dir = (center-pos).normalized
                #print(pos,center,center_dir)
                pos+=center_dir*self.label_dir_mul
                x,y=pos.x,pos.y
                x=int((x+1)/2*self.dc.GetSize().x)
                y=int((-y+1)/2*self.dc.GetSize().y)
                x+=self.label_x_offset
                y+=self.label_y_offset
                #print(x,y)
                self.dc.DrawText(label,x,y)
        
        size = self.GetClientSize()
        x,y=self.dc.GetSize().x,self.dc.GetSize().y
        #x,y=size.width,size.height
        def get_screen_space_pos(x,y):
            x = x / self.dc.GetSize().x * 2 - 1
            y = y / self.dc.GetSize().y * 2 - 1
            y = -y
            return x,y
        def compute_distance(line_length):
            a=Vector4((*get_screen_space_pos(x-50,y-50),0,1))
            b=Vector4((*get_screen_space_pos((x-50-line_length),(y-50)),0,1))
            ...
            a,b=(tuple(a),tuple(b))
            a,b = self._opengl_coord_to_stack(a),self._opengl_coord_to_stack(b)
            distance=self.point_distance_L2(a,b)
            return distance
        
        distance=compute_distance(100)
        distance_ratio=100/distance

        screen_length = -1
        sizes = [500,200,100,50,20,10,5,2,1]
        for size in sizes:
            if distance_ratio*size<200:
                screen_length=size
                break

        #scale drawing
        self.dc.DrawText(f'{screen_length:.0f}',int(x/2+10),int(y-80))
        self.dc.DrawLine(int(x/2),int(y-50),int(x/2+distance_ratio*screen_length),int(y-50))
        self.dc.DrawLine(int(x/2),int(y-40),int(x/2),int(y-60))
        self.dc.DrawLine(int(x/2+distance_ratio*screen_length),int(y-40),int(x/2+distance_ratio*screen_length),int(y-60))
        try:
            ...
            self.dc.DrawText(
    f"""hours from start {self.context.data_context.global_simulation.simulation_time*12:.2f}
    approx. size {self.context.data_context.global_simulation.get_meristem_size():.2f}
    """,30,30)
        except Exception as e:
            print(e)
            #print("drawing simulation data failed")
            pass
    ...
    def _load_concentration_from_file(self, directory,calculate_scale=False):
        ...
        if calculate_scale:
            self.context.data_context.img_TAGS = img.tag
            try:
            ...
            if self.real_distance == None:
                ...
                #model matrix scaling appiled
                self.model_matrix=self.model_matrix#*self.unit_to_microns_coef/100
    ...
    def reset_position(self,scale=2.0):
        self.model_matrix = Matrix44.from_x_rotation(-np.pi / 2)#*self.unit_to_microns_coef/100
        #self.pause=True
    ...
    def init_uniforms(self,shader):
        ...
        self.combined_matrix = matrix44.multiply(self.model_matrix, self.translate)

        #view projection matrix - turn to function possibly
        #vp = matrix44.create_from_scale(np.array((1/self.aspect_ratio,1,1)))
        vp = self.get_vp_matrix()

        glUniformMatrix4fv(vp_loc, 1, GL_FALSE, vp)

        _model_matrix = self.model_matrix*5
        glUniformMatrix4fv(rot_loc, 1, GL_FALSE, _model_matrix)
        glUniformMatrix4fv(trans_loc, 1, GL_FALSE, self.translate)

        if shader == self.skeleton_bundle.shader_sphere:
            glUniform1f(glGetUniformLocation(shader, "aspect_ratio"),self.aspect_ratio)
    ...
    def OnDraw(self):
        ...
        if self.context.data_context.settings.DRAW_ARROWS:
            self.arrows_drawer.draw()
        for draw_func in self.additional_draw:
            draw_func()
    ...
    def OnSkeletonDraw(self):
        ...
        glLineWidth(self.context.data_context.settings.LINE_WIDTH)
        glPatchParameteri(GL_PATCH_VERTICES, 2 )
        glDrawArrays(GL_PATCHES, 0, self.skeleton_bundle.size)
        glDisable(GL_CLIP_PLANE0)
        #glDisable(GL_CULL_FACE)
        if not self.context.data_context.settings.DEPTH_TEST_POINTS:
            glDisable(GL_DEPTH_TEST)
        if self.skeleton_bundle.marked_points:
            glUseProgram(self.skeleton_bundle.shader_sphere)
            init_skeleton_uniform(self.skeleton_bundle.shader_sphere)
            #glPointSize(self.context.data_context.settings.DOT_SIZE)
            glUniform1f(glGetUniformLocation(self.skeleton_bundle.shader_sphere, "point_size"),self.context.data_context.settings.POINT_SIZE)
...
#c.data_context.settings.DRAW_ARROWS=True
class ArrowsDrawer:
    def __init__(self,density_r=10,density_A=10,start=30,end=300):
    ...
    def init_shaders(self):
        ...
        print(glGetShaderInfoLog(gs))
        print(glGetShaderInfoLog(fs))
        self.program_arrow = OpenGL.GL.shaders.compileProgram(
            vs,
            gs,
            fs,
            #validate=False
    ...
    def init_vao(self):
        ...
        self.vbo_points, self.vbo_directions = glGenBuffers(2)
    
    def populate_positions(self):
        self.arrow_size=len(self.positions_opengl)
        glBindVertexArray(self.vao)
        glBindBuffer(GL_ARRAY_BUFFER, self.vbo_points)
        glBufferData(
            GL_ARRAY_BUFFER, self.arrow_size * 3 * 4, np.array(self.positions_opengl, dtype=np.float32).tobytes(), GL_STATIC_DRAW
        )
        glEnableVertexAttribArray(0)
        glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 12, ctypes.c_void_p(0))
        glBindVertexArray(0)
    ...
    def generate_points(self):
        self.positions=list(map(tuple,shape_calculation.generate_cone_points(self.context.data_context.global_simulation.cone_coeffs,Vector3(self.context.data_context.global_simulation.center),10,10,40,300)))
        self.positions_opengl = [self.context.canvas._stack_coord_to_opengl(self.context.canvas._micron_coord_to_stack(x)) for x in self.positions]

    def calculate_directions(self):
        if not self.positions_initialized:
            self.init_points()
            self.positions_initialized=True
        V=self.context.data_context.global_simulation.positions_to_simulation_vectors(self.positions,dict(self.context.data_context.global_simulation.coeffs))
        arrows_directions=[tuple(vv3(self.context.canvas._stack_coord_to_opengl(
                self.context.canvas._micron_coord_to_stack(tuple(vv3(p)+vv3(v)*1))
                ))-vv3(pa)) 
                for pa,p,v in zip(self.positions_opengl,self.positions,V)
            ]
        

        glBindVertexArray(self.vao)
        glBindBuffer(GL_ARRAY_BUFFER, self.vbo_directions)
        glBufferData(
            GL_ARRAY_BUFFER, self.arrow_size * 3 * 4, np.array(arrows_directions, dtype=np.float32).tobytes(), GL_STATIC_DRAW
        )
        # glBufferData(GL_ARRAY_BUFFER, self.size * 3 * 4, colors.tobytes(), GL_STATIC_DRAW)
        glEnableVertexAttribArray(1)
        glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, 12, ctypes.c_void_p(0))
    ...
    def draw(self):
        if not self.positions_initialized:
            self.init_points()
            self.positions_initialized=True
        self.context.canvas.init_uniforms(self.program_arrow)
        glUseProgram(self.program_arrow)
        glBindVertexArray(self.vao)
        glLineWidth(1)
        glDrawArrays(GL_POINTS, 0, self.arrow_size)
        glLineWidth(self.context.data_context.settings.LINE_WIDTH)
...
class PointsMapDrawer:
    def __init__(self,density_r=4,density_A=4,start=30,end=300):
        self.density_r=density_r
        self.density_A=density_A
        self.start=start
        self.end=end
    ...
    def draw_dots(self):
        if not self.positions_initialized:
            ...
            self.positions_initialized=True
        V=self.context.data_context.global_simulation.positions_to_simulation_vectors(self.positions,dict(self.context.data_context.global_simulation.coeffs))
        center = self.context.data_context.global_simulation.center
        colors=[
        cm.viridis(np.arccos(min(0.999,max(0.001,float((vv3(center)-vv3(point)).normalised@vv3(d)))))/np.pi*2)[:3]
            for point,d in zip(self.positions,V)
        ]
        self.context.graphic_context.mark_color_points(self.positions_stack,colors)
        self.context.data_context.settings.DEPTH_TEST_POINTS=True



    def init_points(self):
        self.positions=list(map(tuple,shape_calculation.generate_cone_points(self.context.data_context.global_simulation.cone_coeffs,Vector3(self.context.data_context.global_simulation.center),self.density_r,self.density_A,40,300)))
        P=list(map(tuple,shape_calculation.generate_cone_points(self.context.data_context.global_simulation.cone_coeffs,vv3(self.context.data_context.global_simulation.center)+vv3((0,4,0)),self.density_r,self.density_A,40,300)))
        self.positions_stack = [self.context.canvas._micron_coord_to_stack(x) for x in P]
