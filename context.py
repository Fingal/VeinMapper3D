...
import CIM
from frechet_distance import frdist

import convert_to_tiff_smooth
import wx
...
class Context(Borg):
    def __init__(self):
        ...
        self.graphic_context : GraphicContext
        Borg.__init__(self)
...
class DataContext():
    def __init__(self):
        ...
        self.new_simulation_iterator=None
    ...
    def reload_all(self):
        self.context.skeleton_distances.reload_grid()
        self.context.point_display.reload_inner()
    ...
    def sample_distance(self,first, second,distance=1):
    ...
    #other end is used to send random vein to posistion different than one in original line
    def sample_random_distance(self,first, second,distance=1,percent=90,sigma=30,sample_size=500,other_end=None,center=None):
        ...
        _second=self.context.canvas._stack_coord_to_opengl(other_end)
        ...
        for i in range(sample_size):
            l=generate_line.generate_line_random(_first,_second,sigma,1)
            cuttoff=0
            end = context.canvas._stack_coord_to_micron(first[1])
            if other_end != first[1] and center != None:
                def dist(x,y):
                    return sum((a-b)**2 for a,b in zip(x,y))
                for ii in range(1,len(l)):
                    if dist(center,l[ii])<dist(center,end):
                        cuttoff=ii
                        break
            if cuttoff!=0:
                l=l[:cuttoff]
        ...
        self.context.temporary_paths = temporary_paths
        #self.context.graphic_context.mark_temporary_paths([[(self.context.canvas._micron_coord_to_stack(p)) for p in path] for path in temporary_paths])
        average_line=generate_line.calculate_average_line(temporary_paths)
        print(average_line)
        self.context.graphic_context.mark_temporary_paths([[(self.context.canvas._micron_coord_to_stack(p)) for p in average_line]])
        #self.context.graphic_context.mark_temporary_paths([[(self.context.canvas._micron_coord_to_stack(p)) for p in path] for path in temporary_paths[:30]])
        straight=[]
        for i in range(len(points)+1):
            t=i/len(points)
            straight.append(self.context.canvas.point_line_distance(tuple(a*(1-t)+b*t for a,b in zip(*first)),second))
        ...
        return ([self.context.canvas.point_line_distance(point,second) for point in points],generate_line.calculate_average_distances(random),straight,result,random)
        #return ([self.context.canvas.point_line_distance(point,second) for point in points],straight)
    
    def compare_random_distance(self,first,distance=1,percent=90,sigma=30,sample_size=500,other_end=None,center=None):
        if other_end is None:
            other_end=first[1]
        _t=time.time()
        print("other_end",other_end)
        ...
        for i in range(len(points)+1):
            t=i/len(points)
            straight.append(tuple(a*(1-t)+b*t for a,b in zip(_first,_second)))
        straight_line=[(x,y) for x,y in zip(straight,straight[1:])]
        print(straight)
        temporary_paths=[]
        if center:
            center = context.canvas._stack_coord_to_micron(center)
        for i in range(sample_size):
            ...
            random.append(generate_line.calculate_distance(l,straight_line))
            if i%1==0:
                temporary_paths.append(l)
        print(time.time()-_t)
        print("temporary_paths_set")
        _points=[self.context.canvas._stack_coord_to_micron(x) for x in points]
        #self.context.graphic_context.mark_temporary_paths([[(self.context.canvas._micron_coord_to_stack(p)) for p in path] for path in temporary_paths])
        average_line=generate_line.calculate_average_line(temporary_paths)
        #print(average_line)
        #self.context.graphic_context.mark_temporary_paths([[(self.context.canvas._micron_coord_to_stack(p)) for p in line] for line in (average_line,straight)])
        ...
        return ([self.context.canvas.point_line_segment_distance(self.context.canvas._stack_coord_to_micron(point),straight[0],straight[-1]) for point in points],generate_line.calculate_average_distances(random),straight,result,random)
    ...
    def remove_label(self,point,name=""):
        if point in self.values['label']:
            print(point)
            self.values['label'].remove(point)
            del self.labels['label'][point]
        self.reload_all()
    ...
    def remove(self,values : List[Tuple[int,int,int]],typ):
        #print('VALUES', values)
        for p in values:
            print(p)
                ...
                for start, end in copy.copy(self.values['line']):
                    if start == p or end == p:
                        ...
                        if (start,end) in self.hidden:
                            self.hidden.remove((start,end))
        self.reload_all()
    def move_point(self,old : Tuple[int,int,int],new : Tuple[int,int,int]):
        typ="point"
        if old not in self.values["point"]:
            print("not found")
            return
        #print('VALUES', values)
        self.values["point"][self.values["point"].index(old)]=new
        while old in self.values[typ]:
            self.values[typ].remove(old)
        
        if old in self.labels[typ]:
            self.labels["point"][new]=self.labels["point"][old]
            del self.labels["point"][old]
        if old in self.hidden:
            self.hidden.remove(old)
            self.hidden.add(new)
        if old in self.values["label"]:
            self.values["label"][self.values["label"].index(old)]=new
            self.labels["label"][new]=self.labels["label"][old]
            del self.labels["label"][old]
        self.values
        move_point(self.context.canvas.skeleton_graph,old,new)
        for start, end in copy.copy(self.values['line']):
            if start == old or end == old:
                if start == old:
                    new_line = (new,end)
                if end == old:
                    new_line = (start,new)
                self.values["line"][self.values["line"].index((start,end))]=new_line
                try:
                    self.values['line'].remove((start,end))
                except: 
                    pass
                if (start,end) in self.hidden:
                    self.hidden.remove((start,end))
                    self.hidden.add(new_line)
                self.labels["line"][new_line]=self.labels["line"][(start,end)]
                del self.labels["line"][(start,end)]
        self.reload_all()
            
    def create_skeleton_stack(self, width=3):
    ...
    def create_skeleton_stack_smooth(self,width=3):
        lines,points = self.context.canvas.graph_to_lines_smoothed()
        return convert_to_tiff_smooth.convert_graph_to_tiff(np.array(lines),self.dr_stack_shape,self.context.canvas.unit_to_microns_coef,np.array(self.context.canvas.scaling_ratio),width)
        stack = np.zeros(self.dr_stack_shape,dtype=np.uint16)
        lines,points = self.context.canvas.graph_to_lines_smoothed()
        shape_arr = np.array(self.dr_stack_shape)
        zeros=np.array((0,0,0))
        print(f"lines {len(lines)}")
        for line in lines:
            #i-=1
            line =  [x*self.context.canvas.unit_to_microns_coef for x in line]
            low_corner = np.minimum(*line)-width-1
            high_corner = np.maximum(*line)+width+1
            low_corner_stack=self.context.canvas._micron_coord_to_stack(low_corner)
            high_corner_stack=self.context.canvas._micron_coord_to_stack(high_corner)
            distances = 9999
            poses = []
            ...
            #             poses.append((a,b,c))
            for pos in itertools.product(*[range(int(floor(min(a,b))),int(ceil(max(a,b)))) for a,b in zip(low_corner_stack,high_corner_stack)]):
                arr_pos=np.array(pos)
                if not all(arr_pos<shape_arr) or any(arr_pos<zeros):
                    continue
                pos_micron = np.array(self.context.canvas._stack_coord_to_micron(pos))
                distance,point = generate_line.calculate_closest_point_line(tuple(pos_micron),[[tuple(x) for x in line]])
    ...
    def compute_height_distance(self,point):
        coords = (round(point[0]),round(point[0]),round(point[0]))
        # if notself.check_heightmap_cache(point):
        #     return self.heightmap[coords[0],coords[1]]
        line = self.pi_stack[coords[0],coords[1],:]
        z = line.shape[0]-1
        max_value = np.amax(line[int(ceil(point[2])):])
        start = -1
        end = -1
        for threshold in [0.85,0.8,0.7,0.6]:
            for i in range(z,int(ceil(point[2])),-1):
                if start<0 and np.sum(line[i-2:i+2])/4>max_value*threshold:
                    start = i
                    end = i
                if start>0 and i-start>2 and line[i]<max_value*0.4:
                    end = i
                    break
            if start!=-1:
                break
    ...
    def init_global_simulation(self,labels=[str(i) for i in range(-4,15)],simulation_time=0,is_new_grown=None):
        self.global_simulation.init(self.context.canvas.skeleton_graph,labels,simulation_time=simulation_time,is_new_grown=is_new_grown)
...
class GraphicContext():
    ...
    def reload_color(self):
        if self.marked_color_points[0]:
            self.context.canvas.skeleton_bundle.add_points_color(*self.marked_color_points)
    ...
    def mark_temporary_paths_add(self,paths):
        self.marked_temporary_line+=paths
        points=[]
        for path in paths:
            points.extend(path)
    ...
    def mark_color_points(self,points,colors):
        self.marked_color_points=(self._points_to_opengl_coord(points),colors)
        #print(points)
        self.context.canvas.skeleton_bundle.add_points_color(*self.marked_color_points)
        self.refresh()
    ...
    def mark_temporary_points_add(self,points):
        self.marked_temporary_points+=self._points_to_opengl_coord(points)
        #print(points)
        self.context.canvas.skeleton_bundle.mark_points(self.marked_selection_points+self.marked_temporary_points)
        self.reload_color()
...
class Settings():
    PRINT_MODE = PrintMode.FULL
    CALCULATE_8_DISTANCES=False
    SAVE_IMAGES=True
    SAVE_VECTOR_IMAGES=True
    FULL_HD=False
    DRAW_RING=False
    ...
    DEPTH_TEST_POINTS=False

    POINT_COLOR=(0.7,0.7,0.7)

    DRAW_ARROWS=True
    DRAW_POINTS_MAP=False
