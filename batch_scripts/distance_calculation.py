...
def calculate_distances(path):
    context=Context()
    data_context=context.data_context=DataContext()
    ...
    center = data_context.get_point_from_label("c")
    if not center:
        center = data_context.get_point_from_label("center")
    center = data_context.context.canvas._stack_coord_to_micron(center)

    file_text = ["vein id\tradial distance (2D)\n"]
    
    for i in range(-6,20):
        label = str(i)
        points=list(k for k,v in data_context.labels["label"].items() if v==label)
        if not points:
            file_text.append(f"{label}\t\n")
            continue
        point = data_context.context.canvas._stack_coord_to_micron(points[0])
        distance = ((point[0]-center[0])**2+(point[2]-center[2])**2)**0.5 
        file_text.append(f"{label}\t{distance:.2f}\n")
...
print(files)
for _path in filter(lambda x: "growth.ske" in x, files):
    path = _path[:-3]
