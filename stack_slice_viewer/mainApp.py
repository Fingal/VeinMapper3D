...
class MyApp(wx.App):
    def OnInit(self):
        ...
        # path="C:\\Users\\Andrzej\\Desktop\\2send_1\\"
        # dr_directory=f"{path}dr79-1-7_uncorr_dr.tif"
        # pi_directory=f"{path}dr79-1-7_uncorr_pi.tif"
        
        path="C:\\Users\\Andrzej\\Desktop\\2send_1\\xp5_dr4\\"
        dr_directory=f"{path}dr4_1_dr.tif"
        pi_directory=f"{path}dr4_1_pi.tif"

        self.context.data_context.stack_data.load_concentration(dr_directory, pi_directory)
        print((time.time()-t))
        frame = PositionSetter(self)
        frame.Show()
