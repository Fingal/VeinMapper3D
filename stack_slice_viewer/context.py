...
from stack_data import StackData
...
class DataContext():
    ...
    def compute_heightmap(self):
        if type(self.heightmap) == type(None):
            self.heightmap = get_heightmap(self.pi_stack)
    ...
    def calculate_concentration_height(self,point,depth=10,value=2000):
        height=int(self.heightmap[point[0],point[1]])
    ...
    def calculate_min_concentration_height(self,point,r=3,depth=20,value=2000):
        return min(self.calculate_concentration_height((point[0]+i,point[1]+j),depth,value) for i,j in product(range(-r,r+1),repeat=2))
