...
class StichFrame(wx.Frame):
        ...
        if a.shape[:2]==b.shape[:2]:
            for i in range(target.shape[2]):
                try:
                    ...
                    esential=(a[:,:,s_index]>200).astype(np.uint8)
                    ratio=np.sum(a[:,:,s_index]*esential)/np.sum(b[:,:,t_index]*esential)
                    a_max=np.max(a)
                    b_max=np.max(b)
                    new_ratio=np.sum(np.minimum(a[:,:,s_index],b_max*ratio)*esential)/np.sum(np.minimum(b[:,:,t_index],a_max/ratio)*esential)
    ...
    def combine_arrays(self,source,target,trans_vec,array_history):
        ...
        if maximum:
            ...
            result[t_trans_vec[0]:t_trans_vec[0]+target.shape[0],t_trans_vec[1]:t_trans_vec[1]+target.shape[1],t_trans_vec[2]:t_trans_vec[2]+target.shape[2]]=np.maximum(result[t_trans_vec[0]:t_trans_vec[0]+target.shape[0],t_trans_vec[1]:t_trans_vec[1]+target.shape[1],t_trans_vec[2]:t_trans_vec[2]+target.shape[2]],target)
        else:
            result[s_trans_vec[0]:s_trans_vec[0]+source.shape[0],s_trans_vec[1]:s_trans_vec[1]+source.shape[1],s_trans_vec[2]:s_trans_vec[2]+source.shape[2]]=source
