...
cdef float _calculate_strength(float distance,float age, float pos_range, float neg_range, float inhibition_coef, float attraction_coef,float age_coef, float age_cut_off_coef, float peak_coef):
    ...
    age = max(15,age)
    coef = max(0,(1+age_coef*(age-age_cut_off_coef)))**0.5 
...
cdef float calculate_square_distance(float old_distance,float time_step, float a = 0.114, float b = 2.842, float c = 50.03):
    ...
    cdef float time = (- b + delta**0.5)/(2*a)
...
def global_simulation_step(steps,global_simulation,cone_coeffs,
                            ...
                            growth_rate=30,distance_treshold=(50,4),connection_distance=5,young_attraction_distance=10,young_attraction_strength=0.3,time=1.):
    ...
    cdef Point pos,old_direction,direction,direction2,step,attr_dir
    a = 1
    ...
    for _i in range(steps):
        ...
        for v in global_simulation.lines:
            prediction_lines.append((graph_to_lines(v.skeleton,global_simulation.points),(v.age)))
        #young growth step
        ...
        for i,line in enumerate(global_simulation.in_progres):
            ...
            direction = _predict_step(pos,center, prediction_lines, pos_range, neg_range, inhibition_coef, attraction_coef, age_coef,age_cut_off_coef, straight_coef, peak_coef)
            direction = normalize(add(direction,mul(inertia,old_direction)))
            #young attraction pass
            #TODO more complex function
            for second in global_simulation.in_progres:
                if second!=line and second not in line.connected_secondary:
            ...
            if distance2d(center,step)<np.random.normal(*distance_treshold):
                print('becomes adult',line.age,' at time:',global_simulation.simulation_time)
                line.active=False
                line.start_maturing()
        #connection step
            ...
            for second in global_simulation.in_progres:
                if first.active:
                    ...
                    if distance<connection_distance:
                        ...
                        if closest_point in second.line:
                            first.connection=second.line.index(closest_point)
                        else:
                            first.connection=second.line.index(sorted(second.line,key = lambda x: length(dif(x,closest_point)))[0])
                        print('connecting:',first.age,second.age,'!!!',sep='\n')
                        first.active=False
                        second.connected_secondary.append(first)
                        first.color_index=second.color_index
...
def calculate_error(points, center = (14.785861525386506, 24.7665, -17.67426238150849),a1=0,a2=0,c=0,tan = 2.97):
    return (sum([distance_cone_rotated(point,center,a1,a2,c,tan)**2 for point in points])/len(points))**0.5

def calculate_error_rotated(points_weighted, center,a1=0,a2=0,c=0,tan = 2.97):
    return (sum([distance_cone_rotated(point,center,a1,a2,c,tan)**2*weight for point,weight in points_weighted])/len(points_weighted))**0.5
...
@cython.boundscheck(False) # turn off bounds-checking for entire function
@cython.wraparound(False)  # turn off negative index wrapping for entire function
cdef _fill_array_cone(np.ndarray[DTYPE_t, ndim=3] array,lines,young_lines, float start, float step,
    ...
    for i in range(array.shape[0]):
        ...
        for j in range(array.shape[1]):
            pos = (i*step+start,0,j*step+start)
        ...
        for young_line in young_lines:
            for a,b in young_line:
                ...
                for _ in range(int(ceil(l+1))):
                    ...
                    x = ((a[0]*_step+(1-_step)*b[0])-start)/step
                    y = ((a[2]*_step+(1-_step)*b[2])-start)/step
...
def fill_array_cone_from_lines(int scale, prediction_lines,young_lines,
    ...
    c_min=min(c_min)-50
    c_max=min(c_max)+50
    _fill_array_cone(array, prediction_lines,young_lines, c_min, (c_max-c_min)/scale,
                      pos_range, neg_range, inhibition_coef, attraction_coef, age_coef, age_cut_off_coef, straight_coef, peak_coef, 
                      center, a1, a2, c, tan)
    return array     
    

def fill_array_cone(int scale, global_simulation,
                    float pos_range=0, float neg_range=0, float inhibition_coef=0, float attraction_coef=0,float age_coef=0, float age_cut_off_coef=0, float peak_coef=0,float straight_coef=0,float inertia=0,
                    center=0,float a1=0, float a2=0,float c=0,float tan=0):
    cdef np.ndarray[DTYPE_t, ndim=3] array
    array = np.zeros((scale,scale,3),dtype='float')        
