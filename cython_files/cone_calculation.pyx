...
@cython.boundscheck(False) # turn off bounds-checking for entire function
@cython.wraparound(False)  # turn off negative index wrapping for entire function
cdef _calculate_new_direction(Point direction,int posx, int posy,int posz,float distance, float angle,np.ndarray[DTYPE_t, ndim=3] array,int run_flooding):
    ...
    #
        ...
        to_visit.push(encode((long(posx),long(posy),long(posz))))
...
cdef _calculate_max_direction(int posx, int posy,int posz,float distance,np.ndarray[DTYPE_t, ndim=3] array):
    ...
    while True:
        direction=_RAYS[0]
        _,value = _calculate_new_direction(normalize(direction),posx,posy,posz,distance,0.8,array,False)
...
def calculate_new_direction(np.ndarray[DTYPE_t, ndim=3] array, (int,int,int) pos, (float,float,float) direction, float distance, float angle, int run_flooding=True):
    p,_ = _calculate_new_direction(normalize(Point(direction[0],direction[1],direction[2])),pos[0],pos[1],pos[2],distance,angle,array,bool(run_flooding))
    return (p['x'],p['y'],p['z'])
def calculate_cone(direction,float distance,float angle):
    return _calculate_cone(normalize(Point(direction[0],direction[1],direction[2])),distance,angle)
